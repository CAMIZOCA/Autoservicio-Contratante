    ...
    <div id="innerwrap"> 
            ...
            <div class="articleContentSector">
                ...
                <div class="breadcrumbBox">
                    <ul>
                        ...
                        <li class="last">Incurrido Vs Pagado</li>
                    </ul>
                </div>
                <!-- TÍTULO DEL TEMA / TOPICO -->
                <h1 class="articleTitle">Incurrido Vs Pagado</h1>
                <div class="articleBox">
                    <form  id="form1"  name="form1" action="<?php echo url_for('tralidadincurrvspagado/index') ?>" method="post" enctype="multipart/form-data">
                        <table>                     
                            ...
                            <tr><td >Mes: </td><td colspan="3">                          
                                <select id="mes"  name="mes" onchange="" <?php if (trim($indicador) != 1) { ?> disabled ="true" <?php } ?>>
                                    <option  value="-1"  selected="seleted" > -  Seleccione una opción - </option>
                    ...
                    <div id="showTable" name="show" <?php if (trim($indicador) == '') { ?> style="display:none;"<?php } ?> >                  
                        ...
                        <table class="tableSector" >
                            <thead>
                                <tr align="center">
                                <th width="140" align="center">Mes</th>
                                <th width="105">Monto Incurrido mes Anterior<br />
                                <span style="font-size:11px;">MIP/MA</span></th>
                                <th width="98">Monto Incurrido Total<br />
                                <span style="font-size:11px;">MIT</span></th>
                                <th width="102">Monoto Incurrido Indemnizado<br />
                                <span style="font-size:11px;">MII</span></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                ...
                                foreach ($FONDOS_VW_tabla_inicial as $row_tabla):
                                    $incurrido_pend_ant = str_replace(",", ".", $row_tabla['incurrido_pend_ant']);    //MIP/MA                                             
                                    $incurrido_indemnizado = str_replace(",", ".", $row_tabla['incurrido_indemnizado']); //MII
                                    $incurrido_total = str_replace(",", ".", $row_tabla['incurrido_total']);     // MIT                                         
                                ...
                                endforeach;
                                $total_incurrido_indemnizado2 = $total_incurrido_indemnizado;
                                $total_incurrido_total2 = $total_incurrido_total;
                                $incurrido_pend_ant2 = $incurrido_pend_ant;
      ...
      function drawChart() {
       var data = new google.visualization.DataTable();';
        ...
        data.addColumn('number', 'MIP/MA');
        
        data.addRows([ 
            ['Total '," . $total_incurrido_indemnizado2 . "," . $total_incurrido_total2 . "," . $incurrido_pend_ant2 . "],";
                                ...
                                if ($cuantos_tabla_inicial != 0) {
                                    ...
                                    foreach ($FONDOS_VW_tabla_inicial as $row_tabla):
                                        ...
                                        $var_srcipt = $var_srcipt . "['" . $mes_grafica . "'," . $incurrido_indemnizado2 . "," . $incurrido_total2 . "," . $incurrido_pend_ant2 . "]";
                                        ...
                                        <td class="alignRight"><?php echo number_format(str_replace(",", ".", $incurrido_pend_ant), 2, ",", "."); ?></td>
                                        <td class="alignRight"><?php echo number_format(str_replace(",", ".", $incurrido_total), 2, ",", "."); ?></td>
                                        <td class="alignRight"><?php echo number_format(str_replace(",", ".", $incurrido_indemnizado), 2, ",", "."); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                    ...
                                    ?>
                                    <tr>      
                                    <td><div align="left" style="margin-left:15px; margin-right:10px;" >No existen casos registrados</div></td>
                                ...
                                $var_script2 = $var_srcipt . " ]); var options = {
          width: 600, height: 400,
          title: 'Incurrido vs Pagado',
     ...
     var chart = new google.visualization.ColumnChart(document.getElementById('chart_div'));
        ...
        chart.draw(data, options);
                            ...
                            <tfoot>
                                ...
                                <td><div align="left" style="margin-right:10px; margin-left:15px;" >Total</div></td>
                                <td class="alignRight"><?php echo number_format($total_incurrido_pend_ant, 2, ",", "."); ?></td>                                                   
                        ...
                        ?>
                        <!--FIN--> 
                        <div style="height:420px; margin-left:50px; " align="left">
                        ...
                        <form method="post" id="targetpdf" action="<?php echo url_for('pdf/index') ?>" target="_blank" hidden="hidden" >
                            <input id="titulo_pdf"  name="titulo_pdf" type="text" value="Incurrido Vs Pagado " />
                        ...
                        <form method="post" id="targetexcel" action="<?php echo url_for('excel/index') ?>" target="_blank" hidden="hidden">
                            <input id="titulo"  name="titulo" type="text" value="Incurrido Vs Pagado" />
...
foreach ($CMB_CLIENTE_MVW as $row_c):
    ...
    <?php
    $query = "SELECT ANOPARAM FROM DISPONIBILIDAD_FONDO_VW  J where IDEPOL='$id_pol' and (incurrido_pend_ant<>0 or incurrido_total<>0 or incurrido_indemnizado<>0)
            GROUP BY ANOPARAM ORDER BY ANOPARAM ASC";
    $pdo2 = Doctrine_Manager::getInstance()->getCurrentConnection()->getDbh();
    $stmt2 = $pdo2->prepare($query);
    $stmt2->execute();
